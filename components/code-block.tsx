...
import { CheckIcon, CopyIcon, ChevronDown, ChevronUp } from "lucide-react";
  ...
  return (
    <TooltipProvider delayDuration={100}>
      <div className="relative my-2 rounded-lg border bg-background/60 font-mono text-sm">
        ...
        {/* Code content */}
        <div className={cn("overflow-hidden", collapsed ? "max-h-[360px]" : "max-h-none")}> 
          <pre ref={preRef} className="overflow-x-auto p-3 pt-10 md:p-4 md:pt-12">
            {children}
          </pre>
        </div>
        ...
        {showToggle && (
          <div className="flex justify-center border-t bg-background/60">
            ...
            >
              {collapsed ? (
                <>
                  <ChevronDown className="h-4 w-4" /> Show more ({lineCount - COLLAPSE_AT} lines)
                </>
              ) : (
                <>
