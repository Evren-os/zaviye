...
export default function DataManagementPage() {
  ...
  const [pendingImportData, setPendingImportData] = useState<Array<Omit<Persona, "icon">> | null>(null);
  ...
  const handlePersonaFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        const incoming = Array.isArray(parsed)
          ...
          : Array.isArray(parsed?.personas)
            ? parsed.personas
            : null;
        if (!Array.isArray(incoming)) throw new Error("Invalid personas file format");

        const existing = getRawCustomPersonas();
        const dup = incoming.filter((p: any) => existing.some((e) => e.id === p.id)).length;
        setPendingImportData(incoming);
        setDuplicateCount(dup);
        if (dup > 0) {
          setIsImportDupOpen(true);
        } else {
          const res = importCustomPersonas(incoming as any, { overwrite: false });
  ...
  const confirmResetDefaults = () => {
    try {
      ...
      toast.error("Operation Failed", { description: "Could not reset defaults." });
    } finally {
  ...
  const handleDeletePersonaHistory = (id: string) => {
    try {
      ...
      toast.success("Chat history cleared", { description: `Cleared history for ${id}.` });
    } catch (error) {
      console.error("Failed to delete history for", id, error);
      toast.error("Operation Failed", { description: "Could not delete history for persona." });
    }
  };
  ...
  return (
    <>
      <div className="space-y-6">
        <Card>
          ...
          <CardContent className="divide-y divide-border p-0">
              ...
              control={
                <>
                  <input
                    ...
                    className="hidden"
                  />
                  <Button variant="outline" size="sm" onClick={handleImportPersonasClick}>
                    Import
                  </Button>
                </>
              }
            ...
            <SettingItem
              ...
              isDestructive
              control={
                <Button variant="destructive" size="sm" onClick={handleClearAllLocalData}>
                  Clear
                </Button>
              }
            />
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Delete chat history for a specific persona.</CardDescription>
          </CardHeader>
          <CardContent className="p-0 divide-y divide-border">
            {getAllPersonas().map((persona) => (
              <div key={persona.id} className="flex items-center justify-between p-4">
                <div className="space-y-0.5">
      ...
      <AlertDialog open={isClearAllDataOpen} onOpenChange={setIsClearAllDataOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Clear all local data?</AlertDialogTitle>
            <AlertDialogDescription>
              This will remove all Zaviye data stored in your browser (personas, settings, and
              histories) and reload the app.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction variant="destructive" onClick={confirmClearAllLocalData}>
              Clear
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      ...
      <AlertDialog open={isResetDefaultsOpen} onOpenChange={setIsResetDefaultsOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will remove all custom personas and restore the default model. Chat histories will
              remain intact.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction variant="destructive" onClick={confirmResetDefaults}>
              Reset
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      ...
      <AlertDialog open={isImportDupOpen} onOpenChange={setIsImportDupOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {duplicateCount} duplicate{duplicateCount === 1 ? "" : "s"} detected by id. Merge to
              keep existing, or overwrite to replace duplicates.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
